(function(){
	...
	angular.module('home.publicacionesHome')
		.controller('PublicacionesHome',PublicacionesHome);
    PublicacionesHome.$inject = ['logger','servicios','homeService','$state','$rootScope'];
	function PublicacionesHome(logger,servicios,homeService,$state, $rootScope){
		var vm=this;
        vm.state=false;
        vm.publicacion={
            id:'',
        ...
        vm.cancelar = function(obj){
            ...
            vm.buttons={new:true};
            vm.updates.disabled=true;
            $state.go('Vistas_Home.Vistas_Home_Publicaciones', {id:0});            
        }
        function eliminar(){
            vm.publicacionAux=JSON.parse(JSON.stringify(vm.publicacion));
